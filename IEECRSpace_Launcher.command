#!/bin/bash
# RSpace Interface launcher (macOS). Double-click in Finder to start the app.
...
if [ ! -x "$UV" ] || [ ! -d "$PWD/.venv" ]; then
    ...
    bash Installation/install.sh || { echo "Setup failed."; read -rp "Press Enter to close…"; exit 1; }
fi
